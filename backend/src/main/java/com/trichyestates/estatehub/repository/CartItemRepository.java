...
public interface CartItemRepository extends JpaRepository<CartItem, Long> {
    ...
    @Query("select x.property from CartItem x where x.user.id = :userId order by x.createdAt desc, x.id desc")
    List<Property> findPropertiesByUserId(@Param("userId") Long userId);

    @Modifying
    @Query("delete from CartItem x where x.user.id = :userId and x.property.id = :propertyId")
    int deleteByUserAndProperty(@Param("userId") Long userId, @Param("propertyId") Long propertyId);

    @Modifying
    @Query("delete from CartItem x where x.user.id = :userId")
